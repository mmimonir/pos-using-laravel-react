...
import SupplierDetailsModal from "../suppliers/partials/SupplierDetailsModal";
...
const ShopList = () => {
  ...
  const handleShopDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosInstance
          .delete(`${Constants.BASE_URL}/shop/${id}`)
          .then((res) => {
            setIsLoading(false);
            Swal.fire({
              position: "top-end",
  ...
  return (
    <>
      <BreadCrumb title={"Shop List"} location={"Dashboard"} />
      <div className="row">
